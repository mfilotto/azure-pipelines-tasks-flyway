# Flyway Installer for Azure Devops

This tasks installs a specified version of flyway on the build agent.
** Hosted Ubuntu build agents already have flyway installed. This should only be needed for these agents a different version of flyway is required
...
Ensure that your command line's current directory is set to the flywayInstaller dir
```
cd d:\code\azure-pipelines-tasks-flyway\flywayInstaller
```
...
In order to execute the task locally, a .env file needs to be created. The .env file will set the parameters that are typically set when editing the task in Build or Release pipeline. This file should be created within the root of the FlywayInstaller folder.
...
AGENT_TOOLSDIRECTORY=D:\azure-devops-hostedagent\tools
AGENT_TEMPDIRECTORY=D:\azure-devops-hostedagent\temp

# The version of flyway to install on the build agent
INPUT_FLYWAYVERSION=6.2.0
```
## Compile
...
From the Debug panel, set the configuration to `debug:tf installer` and press F5.
